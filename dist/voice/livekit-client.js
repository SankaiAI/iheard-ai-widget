...
export async function disconnectFromLiveKit() {
  try {
    console.log('🔇 Starting LiveKit disconnect process...');
    ...
    if (voiceActivityDetector) {
      ...
      console.log('🔇 Voice activity detector cleared');
    }
    ...
    if (livekitRoom && localParticipant) {
      ...
      const micTrack = localParticipant.audioTrackPublications.values().next().value?.audioTrack;
      if (micTrack) {
      ...
      localParticipant.audioTrackPublications.forEach((publication) => {
        trackPromises.push(localParticipant.unpublishTrack(publication.track));
      });
    ...
    console.log('✅ LiveKit disconnect completed successfully');
  } catch (error) {
